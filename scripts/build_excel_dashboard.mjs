...
async function createMasterSpreadsheet() {
  const workbook = new ExcelJS.Workbook();
  workbook.creator = 'Marketing Dashboard System';
  workbook.lastModifiedBy = 'Marketing Dashboard System';
  workbook.created = new Date();
  workbook.modified = new Date();

  // Define Headers & Data for all 11 tabs
  const TASK_COLUMNS = [
  ...
  const sheetsDef = [
    { name: 'Tasks', columns: TASK_COLUMNS, data: tasksData },
    { name: 'Projects', columns: MASTER_COLUMNS, data: projectsData },
    { name: 'Vendors', columns: MASTER_COLUMNS, data: vendorsData },
    { name: 'Agencies', columns: MASTER_COLUMNS, data: agenciesData },
    { name: 'Departments', columns: MASTER_COLUMNS, data: departmentsData },
    { name: 'Team Members', columns: MASTER_COLUMNS, data: teamMembersData },
    { name: 'Activity Log', columns: ACTIVITY_LOG_COLUMNS, data: activityLogData },
    { name: 'Users', columns: USER_COLUMNS, data: usersData },
    { name: 'Settings', columns: SETTINGS_COLUMNS, data: settingsData },
    { name: 'Reports', columns: REPORTS_COLUMNS, data: [] },
    { name: 'Dashboard', columns: DASHBOARD_COLUMNS, data: [] }
  ];

  for (const sDef of sheetsDef) {
    const sheet = workbook.addWorksheet(sDef.name, {
      views: [{ state: 'frozen', ySplit: 1 }]
    });

    // Add Header
    const headerRow = sheet.addRow(sDef.columns);
    headerRow.font = { bold: true, color: { argb: 'FF0F172A' }, name: 'Segoe UI' };
    headerRow.fill = {
      type: 'pattern',
      pattern: 'solid',
    ...
    sheet.columns.forEach((column, i) => {
      let maxLen = sDef.columns[i] ? sDef.columns[i].length : 12;
      column.width = Math.min(Math.max(maxLen + 4, 15), 50);
    });
  }

  const file1 = path.join(rootDir, 'Marketing_Task_Dashboard_Master.xlsx');
  const file2 = path.join(rootDir, 'google-sheets-templates', 'Marketing_Task_Dashboard_Master.xlsx');

  await workbook.xlsx.writeFile(file1);
  await workbook.xlsx.writeFile(file2);

  console.log(`Master XLSX generated at: ${file1}`);
}

createMasterSpreadsheet().catch(console.error);
